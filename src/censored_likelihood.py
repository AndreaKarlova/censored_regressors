...
class CensoredGaussianLikelihoodAnalytic(CensoredGaussianLikelihood):
    def __init__(self, variance=None, low=None, high=None, alpha=1., gamma=1., dzeta=1., jitter_=1e-12) -> None:
    ...
    def _expected_log_prob_terms(self, target: Tensor, input: dist.MultivariateNormal, *params: Any, **kwargs: Any) -> Tensor:
        ...
        #print(noise, sigma, mean, variance, std)
        ...
        # upper term
        x_u = 0 if torch.isinf(torch.Tensor([self.high])) else 0.5 * (self.high - 2*target + mean) * std * noise.reciprocal()  # minimized
        upper_term_pdf = standard_normal.log_prob(upper_censor(mean, std)).exp()
        upper_censored_part_cont = x_u * upper_term_pdf

        upper_term_cdf = standard_normal.cdf(-upper_censor(target, sigma))
        # if torch.isinf(self.high) or upper_term_cdf < self.jitter_:
        #   upper_censored_part_atom = 0.
        # else:
        ln_cdf = torch.clamp_min(upper_term_cdf, self.jitter_).log()
        upper_censored_part_atom = ln_cdf * upper_term_pdf  # minimized

        # lower term
        x_l = 0 if torch.isinf(torch.Tensor([self.low])) else 0.5 * (self.low - 2*target + mean) * std * noise.reciprocal()  # maximized
        lower_term_pdf = standard_normal.log_prob(lower_censor(mean, std)).exp()
        lower_censored_part_cont =  x_l * lower_term_pdf

        lower_term_cdf = standard_normal.cdf(lower_censor(target, sigma))
        # if torch.isinf(self.low):
        #   lower_censored_part_atom = torch.zero(lower_term_cdf.shape)
        # else:
